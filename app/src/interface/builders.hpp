...
public:
        ...
        : _element( new Element() )
    {
      ...
      _element->id = id;
      _element->panel = new Panel( num_cols, num_rows );

      for ( u32 i = 0; i < num_cols * num_rows; i++ )
      {
        _element->panel->grid.push_back( { 0, 0, 0, 0 } );
      }
    }
    ...
    {
      _element->panel->fixed_size = true;
      _element->transform.width = width;
      _element->transform.height = height;
      return *this;
    }

    GridPanelBuilder &Children( list<Element *> children )
    {
      _element->panel->children = children;
      return *this;
    }
