...
namespace OverlaySystem {
  ...
  inline void Draw(entt::registry &reg, TextureCache &texture_cache, FontCache &font_cache) {
    ...
    for ( auto ent: provinces ) {
      ...
      if (prov.owner > -1 && prov.settlement != nullptr && prov.settlement->population.current > 0) {
        Rectangle frameRec = { 0.0, 0.0, 128, 128.0 };
        
        DrawTextureRec(
          texture_cache.handle( hstr{ "factionOverlay" } )->texture,
          frameRec,
          prov.tile->position,
          Fade( WHITE, 0.5 ) );
          ...
          DrawTextEx(font_cache.handle(hstr{"font_romulus"})->font, prov.settlement->name, {
