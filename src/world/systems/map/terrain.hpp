...
inline void CreateTerrain( entt::registry &registry ) {
  ...
  for ( u32 i = 0; i < MAP_WIDTH * MAP_HEIGHT; i++ ) {
    ...
    // Tierra
      ...
      tile->biome = c_Tile::LAND;
    //      // Playa
    //      else if (tile->noise >= 0.76f)
    //        tile->biome = BEACH;
    // Mar
    else
      tile->biome = c_Tile::WATER;

    tile->center = { tile->position.x + 64, tile->position.y + 64 };
    tile->coords = { x, y };
    tile->visibility = c_Tile::UNEXPLORED;
...
inline void UpdateFOW( entt::registry &reg ) {
  ...
  for ( auto &entity: view ) {
    ...
    std::shared_ptr<c_Tile::Tile> closest = tiles[closestId];
    u32 x = closest->coords.x;
    u32 y = closest->coords.y;

    std::array<std::shared_ptr<c_Tile::Tile>, 6> neighbors = {
      tiles[index( x, y - 1 )],
      tiles[index( x + 1, y )],
      tiles[index( x, y + 1 )],
      tiles[index( x - 1, y + 1 )],
