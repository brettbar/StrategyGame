...
inline void Draw( entt::registry &registry, bool debug )
{
  entt::basic_view villagers =
    registry.view<Components::Unit, Components::Animated>();

  registry.sort<Components::Unit>(
    []( const Components::Unit &lhs, const Components::Unit &rhs ) {
      return rhs.position.y > lhs.position.y;
    } );

  villagers.each(
    [debug]( Components::Unit &unit, Components::Animated &anim ) {
      //    DrawTextureV(
      //        unit.sprite,
