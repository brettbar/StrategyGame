...
class IManager {
  ...
  void Load() {
    ...
    for (auto const &prov_e: prov_v) {
      ...
      tile_map[map_index(prov_c.tile.coords.x, prov_c.tile.coords.y)] = prov_e;
    }
  }

  bool biome_inhabitable(Biome biome) {
    switch (biome) {
      case Biome::Desert:
      case Biome::Forest:
      case Biome::Hills:
  ...
  NoiseMap olc_gen_perlin_noise(
    ...
    std::cout << "Avg before normalization: " << sum / (MAP_WIDTH * MAP_HEIGHT)
              << "\n";
  ...
  f32 shift_avg(f32 perlin_v, f32 target_avg, f32 scale_factor) {
    return (perlin_v * scale_factor) + (target_avg * (1.0f - scale_factor));
  }
