...
struct Network {
  ...
  inline Network() {
    ...
    if ( not GameNetworkingSockets_Init( nullptr, error_message ) ) {
      std::cerr << "GameNetworkingSockets_Init failed. " << error_message
                << std::endl;
      throw std::runtime_error( "GameNetworkingSockets_Init failed" );
    }
#else
    SteamDatagramClient_SetAppID(
      570 );// Just set something, doesn't matter what
  ...
  inline ~Network() { GameNetworkingSockets_Kill(); }
};
